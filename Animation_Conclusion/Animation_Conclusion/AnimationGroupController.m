...
- (void)viewDidLoad {
    ...
    animateImageTwo.frame = CGRectMake(-200, -64, [UIScreen mainScreen].bounds.size.width + 400, [UIScreen mainScreen].bounds.size.height + 128);
...
- (void)animateStart
{
        ...
        opacityAnimation.duration = 10.0f;
    //移动
        CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"transform.scale"];
        animation.autoreverses = YES;
        animation.fromValue = [ NSValue valueWithCGPoint:self.layImageOne.layer.position];
        CGPoint toPoint = self.layImageOne.layer.position;
        toPoint.x += 200;
        toPoint.y += 128;
        ...
        [self.layImageOne.layer addAnimation:animationGroup forKey:@"animationGroup"];
}

...
-(void)_init
{
    ...
    for (int i = 0; i < 2; i++) {
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(-imageViewsborderOffset, -imageViewsborderOffset, self.bounds.size.width + (imageViewsborderOffset * 2), self.bounds.size.height + (imageViewsborderOffset * 2))];
...
- (void)bringNextImage
{
    ...
    if (animatedType == JLAnimatedType_Translation) {
        [UIView animateWithDuration:self.timePerImage + imageSwappingAnimationDuration + kMovementAndTransitionTimeOffset delay:0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveLinear animations:^{
            NSInteger randomTranslationValueX = imageViewsborderOffset  - [[self class] randomIntBetweenNumber:0 andNumber:imageViewsborderOffset];
